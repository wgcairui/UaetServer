...
export default vue.extend({
  ...
  methods: {
    ...
    async addTerminal() {
      ...
      console.log({
        Terminal,
        mountDevs: Terminal.mountDevs,
        acc: this.accont
      });

      const isPidOccupy = Terminal.mountDevs.find(el => el.pid === arg.pid);
      if (isPidOccupy) {
        const isRegister = await this.$bvModal.msgBoxConfirm(
          `Pid：${isPidOccupy.pid}已经挂载${isPidOccupy.mountDev},是否替换Pid：${isPidOccupy.mountDev}??`,
          { buttonSize: "sm",centered:true }
        );
        if (isRegister) {
    ...
    async delTerminalMountDev(item: TerminalMountDevs) {
      ...
      const isDel: boolean = await this.$bvModal.msgBoxConfirm(
        `确定删除终端绑定设备：Pid(${pid})/${mountDev}??`,
        { buttonSize: "sm",centered:true }
      );
      if (isDel) {
        const result = await this.$apollo.mutate({
