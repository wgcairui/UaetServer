  ...
  <div>
    ...
    <b-form class="p-5">
      <b-form-group label="设备Mac:" label-align-sm="right" label-cols-md="2">
        <b-form-input number trim v-model="accont.DevMac" :state="stateDevMac"></b-form-input>
      </b-form-group>
      ...
      <b-form-group label="注册节点:" label-align-sm="right" label-cols-md="2">
        <b-form-select v-model="accont.mountNode" :options="Nodes.map((el) => el.Name)"></b-form-select>
      </b-form-group>
      <b-form-group label="挂载设备:" label-align-sm="right" label-cols-md="2">
        <b-form-select v-model="accont.mountDev" :options="DevTypes.map((el) => el.DevModel)"></b-form-select>
      </b-form-group>
      <b-form-group label="协议类型:" label-align-sm="right" label-cols-md="2">
        <b-form-select v-model="accont.protocol" :options="Protocols"></b-form-select>
      </b-form-group>
    ...
    <b-table-lite :items="Terminals" :fields="TerminalsFields" responsive>
      ...
      <template v-slot:cell(oprate)="row">
        <b-button-group>
          <b-button variant="danger" @click="deleteTerminal(row.item)">删除终端</b-button>
          <b-button @click="accont.DevMac = row.item.DevMac">修改终端</b-button>
          <b-button variant="info" v-b-modal.addTerminalDev @click="item = row.item.DevMac">添加设备</b-button>
        </b-button-group>
      </template>
    </b-table-lite>
    <b-modal id="addTerminalDev" title="添加挂载设备">
      <b-form>
        ...
        <b-form-group label="挂载设备:" label-align-sm="right" label-cols-md="2">
          <b-form-select v-model="addDev.mountDev" :options="DevTypes.map((el) => el.DevModel)"></b-form-select>
        </b-form-group>
        <b-form-group label="协议类型:" label-align-sm="right" label-cols-md="2">
          <b-form-select v-model="addDev.protocol" :options="addProtocols"></b-form-select>
        </b-form-group>
        <b-form-group label="设备地址：" label-align-sm="right" label-cols-md="2">
