...
export default Vue.extend({
  ...
  methods: {
    async add() {
      const instruct = JSON.parse(
        JSON.stringify(this.$data.instruct)
      ) as OprateInstruct;
      ...
      if (this.$data.isModify) {
        ...
        if (instructExist) {
          const isM = await this.$bvModal.msgBoxConfirm(
            `指令: ${instruct.name} 已经在列表,是否修改`
          );
          if (isM) {
            instructs.forEach((el, index) => {
    ...
    deletes(items: OprateInstruct) {
      this.$data.instructs = (this.$data.instructs as OprateInstruct[]).filter(
        el => el.name !== items.name
      );
    },
    addDevConstent() {
      ...
      this.$apollo
        .mutate({
          ...
          variables: {
            arg,
            Protocol,
            type: "Oprate" as ConstantThresholdType,
            ProtocolType
          }
        })
        .then((res: any) => {
