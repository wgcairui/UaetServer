...
export default vue.extend({
  ...
  computed: {
    ...
    filterProtocols() {
      ...
      return Protocols.filter(el => this.$data.accont.Type === type[el.ProtocolType]).map(el => ({ text: el.Protocol, value: el }));
    }
  },
  watch: {
    statDevType(newVal) {
    ...
    addDevType() {
      this.$apollo.mutate({
        mutation: gql`
          ...
          `,
        variables: { arg: this.$data.accont }
      })
        .then(res => {
      ...
      this.$bvModal.msgBoxConfirm(`确定删除型号"${item.DevModel}"？？？`).then(value => {
        ...
        this.$apollo.mutate({
          mutation: gql`
              ...
              `,
          variables: { DevModel: item.DevModel }
        })
          .then(res => {
